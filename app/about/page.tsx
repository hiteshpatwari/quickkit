...
export default function AboutPage() {
  return (
    <div className="page-wrap prose-page">
      ...
      <section className="architecture-hero" id="architecture"><div><span>01</span><strong>Static registry</strong><p>One typed source powers routes, categories, search, favorites, and the command palette.</p></div><b>→</b><div><span>02</span><strong>Lazy route</strong><p>Each tool is loaded only when opened, keeping the home surface lean.</p></div><b>→</b><div><span>03</span><strong>Worker task</strong><p>Parsing, diffing, conversion, and large-file work move off the main thread.</p></div><b>→</b><div><span>04</span><strong>Local result</strong><p>Outputs return to the interface. Content is never written to product storage.</p></div></section>
      ...
      <section className="about-cta"><div><p className="eyebrow">Try the signature flow</p><h2>Open JSON Formatter and watch the browser do the work.</h2></div><a className="button primary" href="/tools/json-formatter">Open JSON Formatter →</a></section>
    </div>
  );
}
